...
GPT是如下解释关键话的:
> 怎么说呢, 有个非常重要的点就是, 结构化矩阵就是半可分矩阵, 矩阵的秩非常的小,算起来非常的快, 而且可以用到心心念念的矩阵乘法
...
#### 现在在回看一下这个矩阵
![](https://image.chiullian.cn/img/202412052312474.png)
其实就是整体是SSM, 然而,非橘黄色的框矩阵,也就是已经分解后的矩阵, 使用的是"所谓的 `SMA空间结构注意力`", 这些非对角矩阵计算的都是`中间状态`, 可以并行计算(用到了矩阵乘法加速)

而对角矩阵正常算

#### 论文 非常重要的一点就是设计了一个结构化半分注意力 SMA 用来并行计算
### 非常重要的一句话:
> 对角矩阵使用Mamba1里的线性递推来, 而非对角矩阵使用结构化半分注意力 SMA 用来并行计算
> 而且重要的还有下面的 图, 每个非对角矩阵的右上角 也是冗余的,不需要计算的, 使用![](https://image.chiullian.cn/img/202412061400855.png)公式计算
![](https://image.chiullian.cn/img/202412061356980.png)

![](https://image.chiullian.cn/img/202412061350393.png)
